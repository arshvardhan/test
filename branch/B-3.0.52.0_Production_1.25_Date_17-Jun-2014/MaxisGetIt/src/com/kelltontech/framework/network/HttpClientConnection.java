...
//		if( activity != null && ! NativeHelper.isDataConnectionAvailable(activity) ) {
...
//		}
		...
		try {
			...
			Log.i( LOG_TAG, "ServiceResponse Received : " + statusCode );
			...
			/*
			ResponseHandler<String> serviceResponse = new ResponseHandler<String>() {
			...
			if( responseContentStream != null ) {
				...
				serviceResponse.setResponseData(NativeHelper.convertStreamtoBytes(responseContentStream));
			}
				
			if( currentRequest.isCancelled() ) {
				return;
			}
			currentRequest.getServiceController().handleServiceResponse(serviceResponse);
		}
			...
			notifyError("403 Forbidden.", cpex );
		}
		catch (IOException e) {
	...
	private void notifyError( String errorMessage, Exception exception) {
		if( exception == null ) {
			Log.e( LOG_TAG, "Error ServiceResponse: " + errorMessage );
		} else {
			Log.e( LOG_TAG, "Error ServiceResponse: " + errorMessage, exception );
		}
		ServiceResponse serviceResponse = new ServiceResponse();
		serviceResponse.setRequestData(currentRequest.getRequestData());
		serviceResponse.setDataType(currentRequest.getDataType());
		...
		if( exception instanceof ClientProtocolException ) {
			serviceResponse.setHttpResponseCode(403);
		} 
