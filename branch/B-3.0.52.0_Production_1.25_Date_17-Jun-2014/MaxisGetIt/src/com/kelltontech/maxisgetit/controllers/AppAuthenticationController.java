...
public class AppAuthenticationController extends BaseServiceController {
	
	private Context mActivity;
	
	public AppAuthenticationController(IActionController screen, int eventType) {
	...
	public void requestService(Object requestData) {
		try{
			...
			String mobile = (String)requestData;
			String uuid = Utility.getUniqueId(mActivity);
			String modelName = Utility.getDeviceName();
			GenralRequest genralRequest = new GenralRequest(mActivity);
			Hashtable<String, String> urlParams = genralRequest.getAppActivationHeaders(mobile, uuid, modelName);
			...
			String url = AppConstants.BASE_URL + GenralRequest.APP_ACTIVATION_METHOD;
			Log.d("maxis", "url " + url);
			...
			logRequestException(e, "AppAuthenticationController");
	...
	public void responseService(Object object) {
		...
		if (response.isError()) {
			...
			try {
				...
				logResponseException(e, "AppAuthenticationController");
			}
