...
import com.kelltontech.maxisgetit.parsers.TypeByCategoryParser;
...
public class TypeByCategoryController extends BaseServiceController {

	private Context mActivity;

	public TypeByCategoryController(IActionController screen, int eventType) {
	...
	public void requestService(Object requestData) {
		try {
			...
			String mCategoryId = (String) requestData;
			GenralRequest genralRequest = new GenralRequest(mActivity);
			Hashtable<String, String> urlParams = genralRequest.getCategoryheaders(mCategoryId);
			...
			String url = AppConstants.BASE_URL + GenralRequest.TYPE_BY_CATEGORY_Method;
			//String url = "http://192.168.13.16/trunk/ClassifiedRest/" + GenralRequest.TYPE_BY_CATEGORY_Method;
			
			Log.d("maxis TYPE_BY_CATEGORY_Method", "url " + url);
			...
			logRequestException(e, "TypeByCategoryController");
	...
	public void responseService(Object object) {
		...
		if (response.isError()) {
			...
			try {
				response.setPayload(new TypeByCategoryParser().parse(response.getResponseText()));
			} catch (Exception e) {
				logResponseException(e, "TypeByCategoryController");
			}

			if (response.getPayload() instanceof TypeByCategoryResponse) {
				TypeByCategoryResponse tBCResp = (TypeByCategoryResponse) response.getPayload();
				if (tBCResp.isErrorFromServer()) {
					message.obj = tBCResp.getServerMessage();
				} else {
