...
public class FavCompanyListAdapter extends BaseAdapter {
	...
	int count = 0;
	FavCompanyListActivity companyListActivity;

	public FavCompanyListAdapter(Context context, boolean isCompanyType, FavCompanyListActivity activity) {
		mContext = context;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (compDesc.getCompId().equalsIgnoreCase("-1")) {
			...
			int size = (int) TypedValue.applyDimension( TypedValue.COMPLEX_UNIT_DIP, 30, mContext.getResources()
			.getDisplayMetrics()); model.baseLayout .setLayoutParams(new android.widget.RelativeLayout.LayoutParams(
			size, size));
			...
			model.baseLayout.setLayoutParams(new android.widget.RelativeLayout.LayoutParams(android.widget.RelativeLayout.LayoutParams.MATCH_PARENT,
			...
			/*
			 * String city_state=(compDesc.getCity()!=null)?compDesc.getCity():""; if(city_state.trim().length()>0 &&
			 * compDesc.getState().trim().length()>0) city_state+=", "+compDesc.getState(); else
			...
			/*
			 * if(!StringUtil.isNullOrEmpty(compDesc.getAttributes())) model.compAdditionalInfo
			 * .setText(Html.fromHtml(compDesc.getAttributes())); else model.compAdditionalInfo.setText("");
			 */

			if (compDesc.getAttrGroups() != null && compDesc.getAttrGroups().size() > 0) {
				model.compAdditionalInfo.setText(Html.fromHtml("<b>" + compDesc.getAttrGroups().get(0).getLable() + " : </b>"
			...
			if (compDesc.getAttrGroups() != null && compDesc.getAttrGroups().size() > 1) {
				model.compAdditionalInfo2.setText(Html.fromHtml("<b>" + compDesc.getAttrGroups().get(1).getLable() + " : </b>"
			...
			model.box.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					...
					if (count > 0) {
						companyListActivity.textChange(true);
					} else {
						companyListActivity.textChange(false);
					}

				}
			});

			model.box.setChecked(list.get(position).isChecked());
