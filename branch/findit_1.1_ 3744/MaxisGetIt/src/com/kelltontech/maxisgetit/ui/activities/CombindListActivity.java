...
//		initNavigationButton(mClResponse.getPagesCount());
		...
		mCompanyList.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView listView, int firstVisibleItem,int visibleItemCount, int totalItemCount) {
				...
				if(mClResponse!=null){
					if((number)%mClResponse.getRecordsPerPage()==0&&number>0&&number==totalItemCount && totalItemCount<mClResponse.getTotalrecordFound())//mNewsList.size()>totalItemCount
					{	
	...
	/*private void initNavigationButton(int pages) {
		if (pages < 1)
			return;
		if (mPaginationButtons != null) {
			for (int i = 0; i < mPaginationButtons.length; i++) {
				if (mPaginationButtons[i] != null)
					mPaginationButtons[i].setVisibility(View.GONE);
			}
			mMoreLinkView.setVisibility(View.GONE);
		}
		if (pages > 5)
			mMoreLinkView.setVisibility(View.VISIBLE);
		mPaginationButtons = new Button[pages];
		for (int i = 0; i < pages && i < 5; i++) {
			...
			mPaginationButtons[i].setOnClickListener(this);
			if (i == 0) {
	...
	/*private void changeNavigationButtonState(int pages, int currentPage) {
		for (int i = 0; i < pages && i < 5; i++) {
			if (i == currentPage - 1) {
				mPaginationButtons[i].setSelected(true);
				mPaginationButtons[i].setTextColor(getResources().getColor(R.color.white));
				mPaginationButtons[i].setClickable(false);
			} else {
	...
	public void updateUI(Message msg) {
		if (msg.arg2 == Events.COMBIND_LISTING_NEW_LISTING_PAGE || msg.arg2 == Events.USER_DETAIL) {
			...
			if (msg.arg1 == 1) {
				...
				oldResponse.appendCompListAtEnd(mClResponse.getCompanyArrayList());
...
//			break;
		case R.id.col_deal_btn:
			...
			CombinedListRequest newListReq = new CombinedListRequest(CombindListActivity.this);
		...
		case R.id.goto_home_icon:
			...
			Intent intentHome = new Intent(CombindListActivity.this, HomeActivity.class);
