...
//		UiUtils.hideKeyboardOnTappingOutside(findViewById(R.id.rootLayout), this);
	...
	public void onClick(View v) {
		...
		case R.id.upload_btn: {
			...
			AppSharedPreference.putString(AppSharedPreference.MOBILE_NUMBER, phone, getApplicationContext());
		...
		if(!isFetchingBase64)
		{
			new Thread(new Runnable() { 
				...
				public void run() {
					...
					ContestUploadImageController contestUploadImageController=new ContestUploadImageController(ContestUploadImageActivity.this, Events.UPLOAD_IMAGE_EVENT);
					...
					/*requestUploadPhoto.setLatitude(mLatitude==0?mLatitudeN:mLatitude);
				requestUploadPhoto.setLongitude(mLongitude==0?mLongitudeN:mLongitude);*/
	...
	private void manupulateExif()
	{
		try 
		{
			...
			builder.append("Date & Time: " + mExifInterface.getAttribute(ExifInterface.TAG_DATETIME) + "\n\n");
			builder.append("Flash: " + mExifInterface.getAttribute(ExifInterface.TAG_FLASH) + "\n");
			builder.append("Focal Length: " + mExifInterface.getAttribute(ExifInterface.TAG_FOCAL_LENGTH) + "\n\n");
			builder.append("GPS Datestamp: " + mExifInterface.getAttribute(ExifInterface.TAG_FLASH) + "\n");


			String lat=mExifInterface.getAttribute(ExifInterface.TAG_GPS_LATITUDE);
			String lat_ref= mExifInterface.getAttribute(ExifInterface.TAG_GPS_LATITUDE_REF);
			String lon=mExifInterface.getAttribute(ExifInterface.TAG_GPS_LONGITUDE);
			String lon_ref=mExifInterface.getAttribute(ExifInterface.TAG_GPS_LONGITUDE_REF);
			formateLatLongToFloat(lat, lat_ref, lon, lon_ref);

			builder.append("GPS Latitude: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LATITUDE) + "\n");
			builder.append("GPS Latitude Ref: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LATITUDE_REF) + "\n");
			builder.append("GPS Longitude: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LONGITUDE) + "\n");
			builder.append("GPS Longitude Ref: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LONGITUDE_REF) + "\n");
			if(lat==null||lat.length()<=0)
			{
				...
				builder.append("NEW GPS Latitude: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LATITUDE) + "\n");
				builder.append("NEW GPS Latitude Ref: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LATITUDE_REF) + "\n");
				builder.append("NEW GPS Longitude: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LONGITUDE) + "\n");
				builder.append("NEW GPS Longitude Ref: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_LONGITUDE_REF) + "\n");
			}
			/*else
			{
			...
			builder.append("GPS Processing Method: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_PROCESSING_METHOD) + "\n");
			builder.append("GPS Timestamp: " + mExifInterface.getAttribute(ExifInterface.TAG_GPS_TIMESTAMP) + "\n\n");
			builder.append("Image Length: " + mExifInterface.getAttribute(ExifInterface.TAG_IMAGE_LENGTH) + "\n");
			builder.append("Image Width: " + mExifInterface.getAttribute(ExifInterface.TAG_IMAGE_WIDTH) + "\n\n");
			builder.append("Camera Make: " + mExifInterface.getAttribute(ExifInterface.TAG_MAKE) + "\n");
			builder.append("Camera Model: " + mExifInterface.getAttribute(ExifInterface.TAG_MODEL) + "\n");
			builder.append("Camera Orientation: " + mExifInterface.getAttribute(ExifInterface.TAG_ORIENTATION) + "\n");
			builder.append("Camera White Balance: " + mExifInterface.getAttribute(ExifInterface.TAG_WHITE_BALANCE) + "\n");         
		...
		public void handleMessage(android.os.Message msg) 
		{
			if(msg.what==1)
			{
				...
				else if(msg.obj instanceof ResponseUploadPhoto)
				{
					...
					if(photo.isSuccess())
					{
						startActivity(new Intent(ContestUploadImageActivity.this,ContestUploadMoreActivity.class).putExtra("mImagePath", mImagePath).setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT).putExtra(AppConstants.CID_KEY, mCid).putExtra(AppConstants.CATEGORY_ID_KEY,mCategoryId).putExtra(AppConstants.COMPANY_NAME_KEY,mCompanyName));
						mBitmap=null;
						//removeDialog(CustomDialog.PROGRESS_DIALOG);
						stopSppiner();
						System.gc();
			...
			/*else if(msg.what==0)
			{
				...
				if(mLatitude==0&&mLatitudeN==0)
					Toast.makeText(getApplicationContext(), "Could Not Find Image Location.", 1).show();
				else if(mLatitude==0)
				{

					UpdateGeoTag(mLatitudeN, mLongitudeN);
					mUploadBtn.setEnabled(true);
					Toast.makeText(getApplicationContext(), "Could Not Find Image Location Using Gps.", 1).show();
				}
				else
				{
					UpdateGeoTag(mLatitude, mLongitude);
					mUploadBtn.setEnabled(true);
	 ...
	 * @param LONGITUDE_REF
	 */
	private void formateLatLongToFloat(String LATITUDE,String LATITUDE_REF,String LONGITUDE,String LONGITUDE_REF)
	{
