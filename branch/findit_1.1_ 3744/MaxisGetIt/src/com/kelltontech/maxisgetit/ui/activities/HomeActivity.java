...
public class HomeActivity extends MaxisMainActivity {
	...
	private ImageView mHomeIconView, mFacebookIconView, mTwitterIconView, mProfileIconView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<CategoryGroup> categories = bundle.getParcelableArrayList(AppConstants.DATA_CAT_LIST);
		...
		//showDialogWithTitle(getResources().getString(R.string.disclaimer), getResources().getString(R.string.disclaimer_text));
		...
		if (isLocationAware()) {
			setCurrrentLocation();
			Log.i("maxis", "stored " + GPS_Data.getLatitude() + " " + GPS_Data.getLongitude());
	...
	protected void gotoScreen(int position) {
		...
		if (!StringUtil.isNullOrEmpty(cat.getCategoryTitle().trim()) && !StringUtil.isNullOrEmpty(cat.getCategoryId().trim())) {
		HashMap<String,String>	map = new HashMap<String,String>();
		map.put(FlurryEventsConstants.Category_Title, cat.getCategoryTitle().trim());
		map.put(FlurryEventsConstants.Category_Id, cat.getCategoryId().trim());
		...
		if(cat.getCategoryId().equalsIgnoreCase(AppConstants.PHOTO_CONTEST_CAT_ID))
		{
			return;
		}
		if (cat.getMgroupType().trim().equalsIgnoreCase(AppConstants.GROUP_TYPE_CATEGORY))
		{
			if(cat.getmGroupActionType().trim().equalsIgnoreCase(AppConstants.GROUP_ACTION_TYPE_DEAL) || cat.getmGroupActionType().trim().equalsIgnoreCase(AppConstants.GROUP_ACTION_TYPE_LIST)){
				//showDealListing(cat);
				showSubcategories(cat);
			}
			else if(cat.getmGroupActionType().trim().equalsIgnoreCase(AppConstants.GROUP_ACTION_TYPE_CATEGORY_LIST_FOR_GROUP)){
				showCompanyDealListing(cat.getCategoryId(), cat.getCategoryTitle(), cat.getThumbUrl(), true, cat.getMgroupType(), cat.getmGroupActionType());
			}
		...
		else
		{
			if(cat.getmGroupActionType().trim().equalsIgnoreCase(AppConstants.GROUP_ACTION_TYPE_DEAL)){
				...
				showTempletScreen(cat.getCategoryId());
			}
		}
	}
	...
	protected void onStop() {
		...
		/*if(locationReceiver != null)
		{
			try{
				...
				Log.e("maxis", "Unregistered locaiton data");
			}catch(Exception e)	{
				e.printStackTrace();
			}
	...
	protected void onDestroy() {
		...
		if(locationReceiver != null)
		{
			try{
				...
				//AnalyticsHelper.onError(FlurryEventsConstants.LOCATION_RECIEVER_UNREGISTER_ERR, "HomeActivity : " + AppConstants.LOCATION_RECEIVER_UNREGISTER_ERROR_MSG, e);
			}
			locationReceiver = null;
		}
		if(mLocationServiceIntent != null){
			try{
				...
				AnalyticsHelper.onError(FlurryEventsConstants.LOCATION_SERVICE_STOP_ERR, "HomeActivity : " + AppConstants.LOCATION_SERVICE_NOT_STOP_ERROR_MSG, e);
			}
		}
		super.onDestroy();
	}
	...
	private void locationUpdate() {
		...
		//Toast.makeText(HomeActivity.this, "location updated on screen " +  mLattitude + " , " + mLongitude, Toast.LENGTH_LONG).show();
	}
	...
	private void setCurrrentLocation() {
		try {
			...
			AnalyticsHelper.onError(FlurryEventsConstants.CURRENT_LOCATION_NOT_SET_ERR, "HomeActivity : " + AppConstants.CURRENT_LOCATION_NOT_SET_ERROR_MSG, e);
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_photo_contest:
			...
			startActivity(new Intent(HomeActivity.this,ContestHomeActivity.class));
			break;
		default:
			break;
		}
	...
	protected void showTempletScreen(String categoryId) {
		TempletController tempcController = new TempletController(HomeActivity.this, Events.TEMPLET_LISTING);
		startSppiner();
