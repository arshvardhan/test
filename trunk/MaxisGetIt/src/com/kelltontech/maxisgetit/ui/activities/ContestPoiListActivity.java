...
public class ContestPoiListActivity extends ContestBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mListViewPoiByCatOrDist.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView listView, int firstVisibleItem,
				...
				if (BuildConfig.DEBUG) {
					Log.i(LOG_TAG, "first: " + firstVisibleItem + ", visible: "
				...
				startSppiner();
				mPageNumber = totalItemCount
						/ AppConstants.PAGE_SIZE_POI_LIST_BY_DISTANCE + 1;
		...
		mListViewPoiSearch.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView listView, int firstVisibleItem,
				...
				startSppiner();
				mPageNumber = totalItemCount
						/ AppConstants.PAGE_SIZE_POI_LIST_SEACH_RESULTS + 1;
				mIsNextPageRequest = true;
		...
		if (mInitialEventType == Events.POI_SEARCH_EVENT) {
			mResponseListPoiSearch = getIntent().getParcelableExtra(
					AppConstants.LIST_RESPONSE_PARCEL);
			String initialSearchKeyword = getIntent().getStringExtra(
					AppConstants.EXTRA_SEARCH_KEYWORD);
			mSearchKeyEdtTxt.setText(initialSearchKeyword);
		} else {
			mResponseListPoiByCatOrDist = getIntent().getParcelableExtra(
					AppConstants.LIST_RESPONSE_PARCEL);
			mSelectedCategory = getIntent().getParcelableExtra(
					AppConstants.EXTRA_SELECTED_CATEGORY);

			mListAdapterPoiByCatOrDist = new ContestPoiListAdapter(this,
					mResponseListPoiByCatOrDist, mInitialEventType);
		...
		mListAdapterPoiSearch = new ContestPoiListAdapter(this,
				mResponseListPoiSearch, Events.POI_SEARCH_EVENT);
	...
	public void onClick(View v) {
		 ...
		 * Intent(ContestPoiListActivity.this, GetItInfoActivity.class)); break;
		 * }
		 */
		}
	}
